...
public class Clo extends AppCompatActivity {
    EditText CourseCode;
    DBManager db;
    @Override
    ...
    public void createClo(View v)
    {
        String Code = CourseCode.getText().toString();
        ContentValues c = db.getCourseData(Code);
        // Checking if no records found
        if (c == null) {
            showMessage("Error", "No records found against Course Code");
            clearText();
            return;
        }
        else {
            Intent i = new Intent(this, CloCreation.class);
